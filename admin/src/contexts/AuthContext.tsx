...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const isAdmin = (user: User | null): boolean => {
    ...
    // Invite-only system: any authenticated Supabase user is treated as admin.
    // The backend ADMIN_EMAILS env var is the authoritative gate for admin access.
    // To enforce role-based access in the frontend, set role='admin' in Supabase
    // user_metadata via the Admin API or SQL editor.
    logger.debug('isAdmin: granting admin to', user.email, '— backend ADMIN_EMAILS is the real gate');
    return true;
  };
